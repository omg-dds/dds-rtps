...
that cause it to subscribe data with a a different Qos. The expected result may state that all 
data published by one application should be received by the other one in teh correct order without any duplicates.
...
whether the test case *passes* or *fails*s.
